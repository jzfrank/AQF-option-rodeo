...
from consts import DATAROOT, OUTLIER, used_characteristics_all, used_characteristics_nonsparse
from ml_helper_functions import get_data_between, train_validation_test_split


def run_regression_and_save(reg, saved_folder, model_name, option_with_feature, 
    ...
    # print(f"best_alpha: {best_alpha} in {year} for {model_name}")
...
def run_regression_from_to(reg, saved_folder, model_name, option_with_feature, 
                           used_characteristics, start_year=1996, end_year=2012):
    '''
...
def lasso_with_hyperparameter_tuning(saved_folder, 
                                     option_with_feature, used_characteristics, 
                                     year):
    model_name = "Lasso"
    print(datetime.datetime.strftime(datetime.datetime.now(), "%m-%d %H:%M"))
    start = time.time()
    ...
    for alpha in [0.01, 0.1, 0.5, 1]:
        ...
        if best_r2_score is None or r2_score_ > best_r2_score:
            best_alpha = alpha 
            best_r2_score = r2_score_
    print(f"best_alpha: {best_alpha} with \
        r2_score {best_r2_score} in {year} for {model_name}")
    reg = linear_model.Lasso(random_state=0, alpha=best_alpha)

    # regression    
    reg.fit(X_train[used_characteristics], y_train)
    # save model
    joblib.dump(reg, f"{saved_folder}/{model_name}_{best_alpha}_{year}_tuned.pkl")
    with open(f"{saved_folder}/{model_name}_{best_alpha}_{year}_tuned.txt", "w") as f:
        f.write(str(used_characteristics))
...
if __name__ == "__main__":
    ...
    end_year = 2013 
    for year in range(start_year, end_year + 1):
        lasso_with_hyperparameter_tuning(saved_folder, 
                                         option_with_feature, 
                                         used_characteristics_to_train_model,
                                         year)
    ...
    # reg = linear_model.Lasso(random_state=0, alpha=best_alpha)
    # run_regression_from_to(reg, saved_folder, "Lasso_alpha0.1", option_with_feature, used_characteristics_to_train_model, start_year, end_year)
    # # Ridge
    # best_alpha = 0.1  # empirically result from validation
