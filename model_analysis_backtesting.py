...
if __name__ == '__main__':
    ...
    option_with_feature = option_with_feature[
        option_with_feature.option_ret.abs() <= OUTLIER]
    option_with_feature = option_with_feature[
        ~option_with_feature.option_ret.isna()]
    ...
    weight_info = weight_info.rename(
        columns={"dollar_open_interest": "open_interest"})
    print("option_with_feature.shape before merging:", option_with_feature.shape)
    option_with_feature = pd.merge(option_with_feature, weight_info, 
                                   on=["date", "optionid"], how="inner")
    ...
    def run_backtest_and_save(model_root, model_names, 
                              ...
                              start_year=1996, end_year=2013):
        for year in range(start_year, end_year + 1):
            # all models use the same characteristics in the same year 
            with open(Path(model_root, f"{model_names[0]}_{year}.txt"), 
                      "r") as fh:
            ...
            (training_data, validation_data, 
                test_data) = train_validation_test_split(
                option_with_feature, year
            )
            imp = SimpleImputer(missing_values=np.nan, strategy='mean')
            imp.fit(training_data[used_characteristics])
            training_data.loc[:, used_characteristics] = imp.transform(
                training_data[used_characteristics])
            test_data.loc[:, used_characteristics] = imp.transform(
                test_data[used_characteristics])

            for model_name in model_names:
                print(model_root, model_name, saved_folder)
                # load model 
                model = joblib.load(Path(model_root, 
                                         f"{model_name}_{year}.pkl"))
                print(model.get_params())

                dates, gain_from_hedges = backtesting(test_data, model,
                ...
                print(summary_df)
                summary_df.to_csv(Path(saved_folder, 
                                       f"backtest_{model_name}_{year}.csv"))

            # memory management
            print("virtual memory availability before del: ", 
                  (psutil.virtual_memory().available 
                   / psutil.virtual_memory().total * 100)
                  )
            del training_data, validation_data, test_data 
            gc.collect()
            print("virtual memory availability after del: ", 
                  (psutil.virtual_memory().available 
                   / psutil.virtual_memory().total * 100)
                  )
